...
def crop_point_cloud_outside_of_rotated_2d_points(pcd, p1, p2, padding_x=0, padding_y=0, angle=0):
    ...
    crop_point_cloud_outside_of_box(pcd, p_min, p_max)
...
def calc_maximum_distance(pcd, dim):
    ...
    """
    points = np.asarray(pcd.points)
    # Extrahiere die Y-Werte
    y_values = points[:, dim]
    # Berechne die maximale Differenz zwischen den Y-Werten
    max_difference = np.max(y_values) - np.min(y_values)
...
def calculate_street_plane(pcd):
    ...
    p_min = points_reduced_pcd[np.argmin(points_reduced_pcd[:, 0] + points_reduced_pcd[:, 2])]
    # Finde den Punkt mit dem größten x-Wert
    p_max = points_reduced_pcd[np.argmax(points_reduced_pcd[:, 0] + points_reduced_pcd[:, 2])]
    # finde die x-max z-min ecke
    p_min_max = points_reduced_pcd[np.argmax(points_reduced_pcd[:, 0] - points_reduced_pcd[:, 2])]
...
def calculate_average_pothole_depth(pcd, threshold=5):
    ...
    distance = 0
    for point in points:
        if calculate_distance_to_plane(normal, d, point) > distance_threshold:
            added_points += 1
            distance += calculate_distance_to_plane(normal, d, point)
